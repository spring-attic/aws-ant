...
public abstract class AbstractS3Operation implements S3Operation {
	...
	protected S3Scanner getS3Scanner(S3Bucket bucket, PatternSet patterns, String baseDirectory) {
		return new S3Scanner(bucket, baseDirectory, patterns.getIncludePatterns(project), patterns
	...
	/**
	 * Gets a 'directory' that is safe to use with S3. Due to some stupidity in
	 * ANT, a fileset will only pass back a dir argument that is relative to the
	 * project root. This strips that project root part of that directory giving
	 * you the intended path from the root of the bucket.
	 * @param file The ANT supplied directory to be normalized
	 * @return A path that has been stripped of it's project root and made safe
	 * for S3
	 * @throws IOException
	 */
